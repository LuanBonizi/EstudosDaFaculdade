...
void busca_filmes_por_chave_secundaria(FILE *movies, IDXSecundario *headSecundaria, IDXPrimario *head, char *chave){
    IDXSecundario *aux;
    int RRN;
    ...
    while(aux && strcmp(aux->titulo, chave) == 0){
        RRN = busca_RRN(head,aux->chave);//Busca o RRN, dando como chave a chave primária que está no elemento
